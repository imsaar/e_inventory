...
🆕 **One-click order creation from a detail page** - In "Add Order" the form includes an "Import detail page" shortcut: upload an AliExpress order detail webarchive/MHTML and the backend creates the order (number, date, supplier, items, totals, tax) in one step — no manual typing.
